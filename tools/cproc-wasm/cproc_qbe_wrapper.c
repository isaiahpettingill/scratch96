...
int cproc_qbe_main(int argc, char **argv);

static char error_log[8192];
static jmp_buf exit_jmp;
static int exit_status;

void cproc_exit(int status) {
  fflush(NULL);
  exit_status = status;
  longjmp(exit_jmp, 1);
}
...
EMSCRIPTEN_KEEPALIVE
unsigned char *compile_c_to_qbe(const char *source) {
  const char *input_path = "/tmp/scratch96-cproc-input.c";
  const char *output_path = "/tmp/scratch96-cproc-output.ssa";
  const char *error_path = "/tmp/scratch96-cproc-error.log";
  char *argv[] = {
    "cproc-qbe",
    "-t",
    "riscv64",
  ...
  if (write_text_file(input_path, source) < 0) {
    strcpy(error_log, "Failed to write cproc input.");
    return empty_result();
  }
  ...
  if (setjmp(exit_jmp) == 0) {
    status = cproc_qbe_main((int)(sizeof(argv) / sizeof(argv[0])), argv);
  } else {
    status = exit_status == 0 ? 1 : exit_status;
  }
  fflush(stderr);

  if (status != 0) {
    read_error_file(error_path);
    if (error_log[0] == 0) strcpy(error_log, "cproc failed.");
    return empty_result();
  }
