...
export fn read(fd0: c_int, buf: [*:0]u8, nbyte: size_t) isize {
    ...
    const remaining = read_buf.len - read_offset;
    const max_len = if (nbyte > 0) nbyte - 1 else 0;
    const len = @min(remaining, max_len);
    @memcpy(buf[0..len], read_buf[read_offset .. read_offset + len]);
    buf[len] = 0;
    read_offset += len;
